// ClassDashboard.jsx — PATCHED
// Key fix: fetches real students from /api/v1/students?classSection=Grade10-A
// instead of relying on the empty cls.students embedded array.
...
import {
  ...
  Download, Eye, AlertTriangle, Archive, TrendingUp,
  Phone, Mail, MapPin, User, Clock, FileText,
  Link, Video, Image, Paperclip, RefreshCw, ArrowLeft,
  Shield, Award, Zap, ChevronRight, GraduationCap,
  Home, Hash, Save,
} from "lucide-react";
...
const PRIORITY_COLOR = {
  Low: "#64748b", Normal: "#3b82f6", High: "#f97316", Urgent: "#ef4444",
};
...
export default function ClassDashboard() {
  ...
  const [realStudents, setRealStudents] = useState([]); // ← FIX: from students collection
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center px-4">
        ...
        <button onClick={() => navigate("/school-admin/classes")}
          className="mt-2 px-5 py-2.5 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700">
          Go to Classes List
        </button>
      </div>
    );
  ...
  // ── FIX: Fetch real students from students collection ─────────────────────
  const fetchRealStudents = useCallback(async () => {
    if (!cls) return;
    try {
      // Build classSection string: "Grade 10-A" or "10-A"
      const cs = cls.section ? `${cls.name}-${cls.section}` : cls.name;
      const r  = await api.get(`/students`, {
        params: { classSection: cs },
      });
      // /students?classSection=X returns { success, data: [{_id, name, rollNumber, email, classSection}] }
      setRealStudents(r.data.data ?? []);
    } catch (e) {
      console.error("fetchRealStudents error:", e);
      setRealStudents([]);
    }
  }, [cls]);

  useEffect(() => { fetchRealStudents(); }, [fetchRealStudents]);

  const refresh = useCallback(async () => {
    await fetchClass();
  }, [fetchClass]);

  // After class refreshes, also refresh students
  const fullRefresh = useCallback(async () => {
    await fetchClass();
    // fetchRealStudents runs automatically via useEffect when cls updates
  }, [fetchClass]);
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center px-4">
        ...
        <div className="flex gap-3">
          <button onClick={fullRefresh}
            className="px-4 py-2 border border-slate-200 text-sm rounded-xl hover:bg-slate-50">Retry</button>
          <button onClick={() => navigate("/school-admin/classes")}
  ...
  return (
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              {t.id === "students" && (
                <span className="ml-1 bg-indigo-100 text-indigo-600 text-xs px-1.5 py-0.5 rounded-full">
                  {realStudents.length}   {/* ← FIX: real count */}
                </span>
              )}
      ...
      <div className="p-6 max-w-7xl mx-auto">
        {tab === "overview"      && <TabOverview      cls={cls} stats={stats} realStudents={realStudents} />}
        {tab === "students"      && <TabStudents      cls={cls} students={realStudents} post={post} patch={patch} del={del} refresh={fullRefresh} />}
        {tab === "parents"       && <TabParents       students={realStudents} />}
        ...
        {tab === "attendance"    && <TabAttendance    cls={cls} students={realStudents} />}
        {tab === "exams"         && <TabExams         cls={cls} students={realStudents} post={post} patch={patch} del={del} />}
        ...
        {tab === "settings"      && <TabSettings      cls={cls} patch={patch} del={del} navigate={navigate} classId={classId} api={api} refresh={fullRefresh} />}
      </div>
...
// TAB: OVERVIEW
// ─────────────────────────────────────────────────────────────────────────────
function TabOverview({ cls, stats, realStudents }) {
  const s = stats ?? {};
  // Override totalStudents with the live count from students collection
  const totalStudents = s.totalStudents ?? realStudents.length;
          ...
          <div className="space-y-2">
            {cls.assignments.slice(-5).reverse().map((a) => (
              <div key={a._id} className="flex items-center justify-between py-2.5 border-b border-slate-50 last:border-0">
                ...
                <Badge
                  label={a.status}
                  color={a.status === "Active" ? "#6366f1" : a.status === "Graded" ? "#10b981" : "#94a3b8"}
                />
              </div>
            ))}
          </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// TAB: STUDENTS — now receives real students from students collection
// ─────────────────────────────────────────────────────────────────────────────
function TabStudents({ cls, students, post, patch, del, refresh }) {
  const [search, setSearch] = useState("");

  const filtered = students.filter((s) =>
    (s.name || "").toLowerCase().includes(search.toLowerCase()) ||
    (s.rollNumber || s.rollNo || "").toLowerCase().includes(search.toLowerCase())
  );
        ...
        <div className="text-center py-16 text-slate-400">
          ...
          {students.length === 0 && (
            <p className="text-xs mt-2 text-slate-300">
              Enroll students via the Students module — assign them to this grade &amp; section.
            </p>
          )}
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                {["#", "Roll No", "Name", "Email", "Class"].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((s, i) => (
                <tr key={s._id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-500 text-xs">{s.classSection || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
// ─────────────────────────────────────────────────────────────────────────────
// TAB: PARENTS — reads guardian info from student docs (via API enrichment)
// ─────────────────────────────────────────────────────────────────────────────
function TabParents({ students }) {
  const [search, setSearch] = useState("");
  // Students from /students?classSection=X have minimal fields.
  // Show what we have; if parentName etc. aren't in the response, show a note.
  const withParents = students.filter((s) => s.parentName || s.parentPhone);
  const noParentData = withParents.length === 0;
    ...
    <div className="space-y-5">
      {noParentData ? (
        <div className="text-center py-16 text-slate-400">
          ...
          <p className="font-medium">Parent data is stored in student profiles</p>
          <p className="text-xs mt-2 text-slate-300">
            Visit the Students module to view and edit guardian/parent information.
          </p>
        </div>
      ) : (
        <>
              ...
              <div key={s._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-indigo-500">Roll #{s.rollNumber || s.rollNo || "N/A"}</p>
                  </div>
                </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {subjects.map((s, i) => (
                <tr key={i} className="hover:bg-slate-50 group transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className="font-mono text-xs bg-slate-100 px-2 py-1 rounded-lg">{s.subjectCode || "—"}</span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <Badge label={s.isElective ? "Elective" : "Core"} color={s.isElective ? "#f59e0b" : "#6366f1"} />
                  </td>
                  <td className="px-4 py-3">
        ...
        <Modal title={editIdx !== null ? "Edit Subject" : "Add Subject"} onClose={() => setModal(false)}>
          <div className="space-y-4">
            <Field label="Subject Name" required>
              <input value={form.subjectName ?? ""} onChange={(e) => setForm((p) => ({ ...p, subjectName: e.target.value }))} className={inp(false)} />
            </Field>
            <Field label="Subject Code">
              <input value={form.subjectCode ?? ""} onChange={(e) => setForm((p) => ({ ...p, subjectCode: e.target.value }))} className={inp(false)} />
            </Field>
            <Field label="Assigned Teacher">
              <input value={form.teacherName ?? ""} onChange={(e) => setForm((p) => ({ ...p, teacherName: e.target.value }))} className={inp(false)} />
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Max Marks">
                <input type="number" value={form.maxMarks ?? 100} onChange={(e) => setForm((p) => ({ ...p, maxMarks: e.target.value }))} className={inp(false)} />
              </Field>
              <Field label="Pass Marks">
                <input type="number" value={form.passMarks ?? 33} onChange={(e) => setForm((p) => ({ ...p, passMarks: e.target.value }))} className={inp(false)} />
              </Field>
            </div>
            <Field label="Type">
...
// ─────────────────────────────────────────────────────────────────────────────
// TAB: TIMETABLE  (unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
              ...
              <div className="flex flex-wrap gap-2 p-4">
                {slotsForDay(day).map((s, i) => (
                  <div key={s._id || s.tempId || i}
                    ...
                    <div className="w-1.5 h-full rounded-full absolute left-0 top-0 bottom-0"
                      style={{ background: TYPE_COLOR[s.type] || "#6366f1" }} />
                    <div className="ml-2">
...
// ─────────────────────────────────────────────────────────────────────────────
// TAB: ATTENDANCE — uses real students
// ─────────────────────────────────────────────────────────────────────────────
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setSaved(true)}
              className="px-4 py-2 text-xs bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-semibold flex items-center gap-1.5">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {students.map((s) => {
                ...
                return (
                  <tr key={s._id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 text-indigo-500 font-semibold text-xs">
                      {s.rollNumber || s.rollNo || "—"}
                    </td>
...
// ─────────────────────────────────────────────────────────────────────────────
// TAB: EXAMS & RESULTS — uses real students for pre-populating results
// ─────────────────────────────────────────────────────────────────────────────
function TabExams({ cls, students, post, patch, del }) {
  ...
  const openResults = (exam) => {
    setSelExam(exam);
    // FIX: if exam.results is empty, seed from real students
    const seeded = exam.results?.length
      ? exam.results.map((r) => ({ ...r }))
      : students.map((s) => ({
          studentId:     s._id,
          studentName:   s.name,
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-600">Exams ({cls.exams?.length ?? 0})</h3>
        <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm rounded-xl hover:bg-indigo-700 font-semibold">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-slate-400 flex-wrap">
                      ...
                      {exam.date && <span>{fmt(exam.date)}</span>}
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <button onClick={() => openResults(exam)}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg hover:bg-indigo-50 text-indigo-500 text-xs font-semibold">
        ...
        <Modal title="Create Exam" onClose={() => setModal(false)}>
          <div className="space-y-4">
            <Field label="Exam Name" required>
              <input value={form.name || ""} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} className={inp(false)} placeholder="e.g. Unit Test 1" />
            </Field>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Type">
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
            <button onClick={() => setModal(false)} className="px-4 py-2 text-sm border border-slate-200 rounded-xl hover:bg-slate-50">Cancel</button>
            <button onClick={createExam} className="px-5 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-semibold">Create</button>
          </div>
        </Modal>
      )}
      ...
      {modal === "results" && selExam && (
        <Modal title={`Results — ${selExam.name}`} onClose={() => setModal(false)} wide>
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {["Roll", "Student", `Marks / ${selExam.maxMarks}`, "Grade", "Absent", "Remarks"].map((h) => (
                    <th key={h} className="text-left px-3 py-2.5 text-xs font-bold text-slate-500 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {results.map((r, i) => (
                  <tr key={i} className={r.isAbsent ? "opacity-40" : ""}>
                    ...
                    <td className="px-3 py-2 text-sm font-medium text-slate-700">{r.studentName}</td>
                    <td className="px-3 py-2">
                      <input type="number" value={r.marksObtained || 0} disabled={r.isAbsent}
                        onChange={(e) => {
                          const m = Number(e.target.value);
                          updateResult(i, "marksObtained", m);
                          updateResult(i, "grade", calcGrade(m, selExam.maxMarks, selExam.passMarks));
                        }}
                    ...
                    <td className="px-3 py-2">
                      <span className="text-sm font-bold" style={{ color: gradeColor[r.grade] || "#94a3b8" }}>{r.grade || "—"}</span>
                    </td>
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
            <button onClick={() => setModal(false)} className="px-4 py-2 text-sm border border-slate-200 rounded-xl hover:bg-slate-50">Cancel</button>
            <button onClick={saveResults} className="px-5 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-semibold">
              <Save size={14} className="inline mr-1.5" />Save Results
            </button>
          </div>
        </Modal>
      )}
...
function TabAnnouncements({ cls, post, patch, del }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-600">Announcements ({sorted.length})</h3>
        <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm rounded-xl hover:bg-indigo-700 font-semibold">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Expires On">
                <input type="date" value={form.expiresAt?.split("T")[0] || ""} onChange={(e) => setForm((p) => ({ ...p, expiresAt: e.target.value }))} className={inp(false)} />
              </Field>
            </div>
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="text-sm text-slate-600 font-medium">Pin this announcement</span>
            </label>
          </div>
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
            <button onClick={() => setModal(false)} className="px-4 py-2 text-sm border border-slate-200 rounded-xl hover:bg-slate-50">Cancel</button>
            <button onClick={handleSave} className="px-5 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-semibold">
              <Save size={14} className="inline mr-1.5" />{editId ? "Save" : "Post"}
            </button>
          </div>
        </Modal>
      )}
...
function TabResources({ cls, post, del }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-600">Resources ({cls.resources?.length ?? 0})</h3>
        <button onClick={() => { setForm({ type: "Document" }); setModal(true); }}
          className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white text-sm rounded-xl hover:bg-indigo-700 font-semibold">
            ...
            return (
              <div key={r._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 group hover:border-indigo-100 transition-all flex items-start gap-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{ background: (TYPE_COLOR[r.type] || "#94a3b8") + "18" }}>
                ...
                <button onClick={() => del(`resources/${r._id}`)}
                  className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                  <Trash2 size={13} />
                </button>
              </div>
            );
      ...
      {modal && (
        <Modal title="Add Resource" onClose={() => setModal(false)}>
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
            <button onClick={() => setModal(false)} className="px-4 py-2 text-sm border border-slate-200 rounded-xl hover:bg-slate-50">Cancel</button>
            <button onClick={handleSave} className="px-5 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-semibold">Add</button>
          </div>
        </Modal>
      )}
    </div>
...
function TabSettings({ cls, patch, navigate, classId, api, refresh }) {
  const [editForm, setEditForm] = useState({
    ...
    classTeacherName: cls.classTeacherName, room: cls.room,
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[["name", "Class Name"], ["section", "Section"], ["academicYear", "Academic Year"],
            ["classTeacherName", "Class Teacher"], ["room", "Room"]].map(([k, lbl]) => (
            <Field key={k} label={lbl}>
              <input value={editForm[k] ?? ""} onChange={(e) => setEditForm((p) => ({ ...p, [k]: e.target.value }))} className={inp(false)} />
            </Field>
          ))}
          <Field label="Capacity">
            <input type="number" value={editForm.capacity ?? ""} onChange={(e) => setEditForm((p) => ({ ...p, capacity: e.target.value }))} className={inp(false)} />
          </Field>
        </div>
        <div className="mt-4">
          <Field label="Notes">
        ...
        <button onClick={saveInfo} disabled={saving}
          className="mt-4 flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white text-sm rounded-xl hover:bg-indigo-700 disabled:opacity-50 font-semibold">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        ...
        <p className="text-xs text-slate-400 mb-4">Move all students to the next academic class.</p>
        <div className="flex gap-3">
          <input value={promoteTo} onChange={(e) => setPromoteTo(e.target.value)}
            className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-violet-400"
            placeholder="Promote to (e.g. Grade 11)" />
          <button onClick={doPromote} disabled={saving || cls.isPromoted}
            className="px-4 py-2 text-sm bg-violet-600 text-white rounded-xl hover:bg-violet-700 disabled:opacity-50 font-semibold">
      ...
      <div className={`bg-white rounded-2xl border shadow-sm p-6 ${cls.isArchived ? "border-amber-100" : "border-red-100"}`}>
        ...
        {cls.isArchived ? (
          <button onClick={doRestore} disabled={saving}
            className="px-5 py-2.5 text-sm bg-amber-500 text-white rounded-xl hover:bg-amber-600 disabled:opacity-50 font-semibold">
            Restore Class
          </button>
        ) : (
          <button onClick={doArchive} disabled={saving}
            className="px-5 py-2.5 text-sm bg-red-500 text-white rounded-xl hover:bg-red-600 disabled:opacity-50 font-semibold">
            Archive Class
          </button>
        )}
      </div>
