...
import {
  Plus, Search, X, Filter, Clock, Users, TrendingUp, FileText,
  CheckCircle2, AlertCircle, Archive, Edit2, Trash2, Send,
  MoreVertical, ChevronDown, Loader2, BookOpen, GraduationCap,
  User, Link as LinkIcon, CheckCircle, Upload, Tag, Save, ClipboardList,
} from "lucide-react";
...
const EMPTY = {
  ...
  assignmentType: "homework", submissionType: "file",
  allowLateSubmission: false, latePenalty: 10,
  rubric: [], attachments: [], tags: [],
  priority: "normal", estimatedTime: 60, difficulty: "medium",
  learningObjectives: [], resources: [],
  notifyStudents: true, notifyParents: false,
};
...
export default function TeacherAssignments() {
  ...
  // Extract primitive IDs at render time — NOT inside useCallback
  // user._id is a plain string; user.schoolId is an object {_id, name, ...}
  const teacherId = String(user?._id?._id ?? user?._id ?? "");
  const schoolId  = String(user?.schoolId?._id ?? user?.schoolId ?? "");
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const assignments =
        Array.isArray(aR.data?.data?.data) ? aR.data.data.data :
        Array.isArray(aR.data?.data)       ? aR.data.data       :
        Array.isArray(aR.data)             ? aR.data            : [];
      const statsData =
        sR.data?.data?.data ?? sR.data?.data ?? sR.data ??
        { total: 0, published: 0, drafts: 0, overdue: 0 };
      setList(assignments);
      setStats(statsData);
    } catch (err) {
      toast.error(err.response?.data?.message ?? "Could not load assignments.");
      setList([]);
      setStats({ total: 0, published: 0, drafts: 0, overdue: 0 });
    } finally {
  ...
  // KEY FIX: teacherId and schoolId are primitive strings extracted at render
  // time above, so this callback always has the correct resolved values.
  const fetchOpts = useCallback(async () => {
    if (!teacherId || !schoolId) {
      // Auth not ready — use fallbacks so drawer is still usable
    ...
    try {
      const [sR, cR] = await Promise.all([
        schoolAPI.getTeacherSubjects(teacherId),
        schoolAPI.getSchoolClasses(schoolId),
      ]);

      const subPayload   = sR.data?.data?.data ?? sR.data?.data ?? {};
      const classPayload = cR.data?.data?.data ?? cR.data?.data ?? {};

      const fetchedSubjects = (subPayload.subjects ?? []).map(s =>
        typeof s === "string" ? s : (s.name ?? String(s))
      );
      const fetchedClasses = (classPayload.classes ?? []).map(c =>
        typeof c === "string" ? c : (c.name ?? String(c))
      );
      const fetchedSections = (classPayload.sections ?? []).map(s =>
        typeof s === "string" ? s : String(s)
      );

      // Always fall back to defaults if API returns empty arrays
      setSubjects(fetchedSubjects.length > 0
        ? fetchedSubjects
        : ["Mathematics","Science","English","Hindi","Social Studies","Computer Science"]);
      setClasses(fetchedClasses.length > 0
        ? fetchedClasses
        : ["Grade 6","Grade 7","Grade 8","Grade 9","Grade 10","Grade 11","Grade 12"]);
      setSections(fetchedSections.length > 0
        ? fetchedSections
        : ["A","B","C","D"]);
  ...
  const openEdit = (a) => {
    ...
    setForm({
      title:               a.title,
      subject:             a.subject,
      class:               a.class    ?? a.grade,
      section:             Array.isArray(a.section)   ? a.section   :
                           Array.isArray(a.sections)  ? a.sections  : [],
      description:         a.description  ?? "",
      instructions:        a.instructions ?? "",
      dueDate:             a.dueDate?.slice(0, 10) ?? "",
      dueTime:             a.dueTime ?? "23:59",
      totalMarks:          a.points  ?? a.maxMarks ?? 100,
      passingMarks:        a.passingMarks ?? 40,
      assignmentType:      a.assignmentType ?? a.type ?? "homework",
      submissionType:      a.submissionType ?? "file",
      ...
      estimatedTime:       a.estimatedTime ?? 60,
      difficulty:          a.difficulty    ?? "medium",
      learningObjectives:  a.learningObjectives ?? [],
      resources:           a.resources     ?? [],
      notifyStudents:      a.notifyStudents ?? true,
      notifyParents:       a.notifyParents  ?? false,
    });
  ...
  const toggleSec = (s) =>
    setForm(p => ({
      ...p,
      section: p.section.includes(s)
        ? p.section.filter(x => x !== s)
        : [...p.section, s],
    }));
  ...
  const addObj = () => {
    if (obj.trim()) {
      setForm(p => ({ ...p, learningObjectives: [...p.learningObjectives, obj.trim()] }));
      setObj("");
    }
  };
  const addRes = () => {
    if (res.title && res.url) {
      setForm(p => ({ ...p, resources: [...p.resources, { ...res, id: Date.now() }] }));
      setRes({ title: "", url: "" });
    }
  };
  const addRub = () => {
    if (rub.criteria && rub.points) {
      setForm(p => ({
        ...p,
        rubric: [...p.rubric, { criteria: rub.criteria, points: +rub.points, description: rub.description }],
      }));
      setRub({ criteria: "", points: "", description: "" });
    }
  };
  const onFiles = (e) => {
  ...
  const validate = () => {
    if (!form.title.trim()) {
      toast.error("Enter a title"); setTab("basic"); return false;
    }
    if (!form.subject) {
      toast.error("Select a subject"); setTab("basic"); return false;
    }
    if (!form.class) {
      toast.error("Select a class"); setTab("basic"); return false;
    }
    if (!form.section || form.section.length === 0) {
      toast.error("Select at least 1 section"); setTab("basic"); return false;
    }
    if (!form.dueDate) {
      toast.error("Set a due date"); setTab("details"); return false;
    }
    if (+form.passingMarks > +form.totalMarks) {
      toast.error("Passing marks exceed total marks"); setTab("details"); return false;
    }
    return true;
  };
  ...
  const saveDraft = async () => {
    ...
    try {
      if (mode === "edit") {
        ...
        const updated = r.data?.data?.data ?? r.data?.data ?? null;
        if (updated) setList(p => p.map(a => (a.id ?? a._id) === editId ? updated : a));
      } else {
        const r = await assignmentAPI.create(mkPayload("draft"));
        const created = r.data?.data?.data ?? r.data?.data ?? null;
  ...
  const publish = async () => {
    ...
    try {
      if (mode === "edit") {
        const r = await assignmentAPI.update(editId, mkPayload("published"));
        const updated = r.data?.data?.data ?? r.data?.data ?? null;
        if (updated) setList(p => p.map(a => (a.id ?? a._id) === editId ? updated : a));
      } else {
        const r = await assignmentAPI.create(mkPayload("published"));
        const created = r.data?.data?.data ?? r.data?.data ?? null;
        if (created) setList(p => [created, ...p]);
        setStats(s => ({ ...s, total: s.total + 1, published: s.published + 1 }));
      }
      toast.success(mode === "edit" ? "Assignment updated!" : "Assignment published!");
      setOpen(false);
    } catch (err) {
  ...
  const quickPublish = async (id) => {
    try {
      ...
      setStats(s => ({ ...s, drafts: Math.max(0, s.drafts - 1), published: s.published + 1 }));
      toast.success("Assignment published!");
    } catch {
      toast.error("Could not publish assignment");
    }
    setMenuId(null);
  };

  // ── Derived ───────────────────────────────────────────────────────────────
  const allSubjects  = ["all", ...new Set(list.map(a => a.subject).filter(Boolean))];
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="relative flex items-center gap-2 px-5 py-2.5 bg-white border-2 border-amber-400 text-amber-600 rounded-xl font-semibold text-sm hover:bg-amber-50 active:scale-95 transition-all"
            >
              <ClipboardList size={17} strokeWidth={2.5} />
              Pending Review
              {pendingCount > 0 && (
                <span className="absolute -top-2 -right-2 min-w-[20px] h-5 px-1 bg-amber-500 text-white text-xs font-bold rounded-full flex items-center justify-center">
            ...
            <button
              type="button"
              onClick={openCreate}
              className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-semibold text-sm hover:bg-indigo-700 active:scale-95 transition-all shadow-md shadow-indigo-200"
            >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <div className="relative">
                          <button type="button" onClick={() => setMenuId(p => p === id ? null : id)}
                          ...
                          {menuId === id && (
                            <div className="absolute right-0 top-8 z-30 w-44 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                              ...
                              {status === "draft" && (
                                <MI icon={<Send size={14} />}        label="Publish"          color="text-indigo-600" onClick={() => quickPublish(id)} />
                              )}
                              <MI icon={<ClipboardList size={14} />} label="View Submissions" color="text-amber-600"
              ...
              {tab === "basic" && (
                <>
                  ...
                  <FL label="Sections" req>
                    {optsLoad ? (
                      <div className="flex items-center gap-2 text-sm text-gray-400">
                        <Loader2 size={13} className="animate-spin" /> Loading sections…
                      </div>
                    ) : (
                      <div className="flex flex-wrap gap-2">
                        ...
                        {sections.length === 0 && (
                          <p className="text-xs text-gray-400 italic">No sections found — contact admin.</p>
                        )}
                      </div>
                    )}
                    {form.section.length > 0 && (
                      <p className="text-xs text-indigo-600 mt-1.5 font-medium">Selected: {form.section.join(", ")}</p>
                    )}
                  </FL>
                  ...
                  <FL label="Description">
                    <textarea name="description" value={form.description} onChange={fc} rows="2"
                  ...
                  <FL label="Tags">
                    <div className="flex gap-2 mb-2">
                      ...
                      <button type="button" onClick={addTag} className="px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                        <Plus size={16} />
                      </button>
                    </div>
              ...
              {tab === "details" && (
                <>
                  <FL label="Detailed Instructions">
                    <textarea name="instructions" value={form.instructions} onChange={fc} rows="4"
                  ...
                  <FL label="Learning Objectives">
                    <div className="flex gap-2 mb-2">
                      <input value={obj} onChange={e => setObj(e.target.value)}
                        ...
                        className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                      <button type="button" onClick={addObj} className="px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                        <Plus size={16} />
                      </button>
                    </div>
                    {form.learningObjectives.map((o, i) => (
                      <div key={i} className="flex items-center gap-2 p-2.5 bg-gray-50 rounded-lg mb-1">
                        ...
                        <button type="button"
                          onClick={() => setForm(p => ({ ...p, learningObjectives: p.learningObjectives.filter((_,j) => j !== i) }))}
                    ...
                    <div className="space-y-2">
                      {form.rubric.map((r, i) => (
                        <div key={i} className="flex items-start justify-between p-3 bg-gray-50 rounded-lg">
                          ...
                          <button type="button"
                            onClick={() => setForm(p => ({ ...p, rubric: p.rubric.filter((_,j) => j !== i) }))}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <div className="flex gap-2">
                      <input value={rub.description} onChange={e => setRub(p => ({ ...p, description: e.target.value }))}
                        ...
                        className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                      <button type="button" onClick={addRub} className="px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shrink-0">
                        <Plus size={16} />
                      </button>
                    </div>
                  </div>
                  ...
                  <FL label="Resource Links">
                    <div className="flex gap-2 mb-2">
                      ...
                      <input type="url" value={res.url} onChange={e => setRes(p => ({ ...p, url: e.target.value }))}
                        placeholder="https://…"
                        className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                      <button type="button" onClick={addRes} className="px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shrink-0">
                        <Plus size={16} />
                      </button>
                    </div>
                    {form.resources.map(r => (
                      <div key={r.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg mb-1">
                        ...
                        <button type="button"
                          onClick={() => setForm(p => ({ ...p, resources: p.resources.filter(x => x.id !== r.id) }))}
                          className="text-red-400 hover:text-red-600"><X size={14} /></button>
                      </div>
                    ))}
                  </FL>
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button type="button" onClick={publish} disabled={saving}
                  className="flex items-center gap-2 px-5 py-2 bg-indigo-600 text-white rounded-xl font-semibold text-sm hover:bg-indigo-700 disabled:opacity-50 shadow-md shadow-indigo-200 active:scale-95 transition-all">
